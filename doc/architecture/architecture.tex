...
\begin{tikzpicture}
  ...
  \node[our code, above=.5 of event] (portal) {Portal};
  ...
  \begin{scope}[node distance=.7,
    ...
    \node[below=of oncologist.east, anchor=east] (data manager) {Data Manager};
  ...
  \draw[<->, out=180,in=-90] (output) to node[edge label, above, near
  end] {\CCD} (holon);
